...
contour bound `γ_Λ(|σ_a − ω^z_a| ≥ k | ω^z) ≤ 2r'(β/2)^k`, reusing the anchored-circuit count of the
sharp Peierls estimate. Where Georgii's series converges below one (`β > 2.416`) this beats his
`r(β)^k` by `6^k/2`; below that his bound is truncated at one and ours is the weaker of the two. **(6.21)**: for `β ≥ log 12` every slope gives
a Gibbs measure pinned to its staircase with probability above one half, distinct slopes give
distinct measures, and each differs from its images under the spin translation, the spin
reflection, the shifts along the first coordinate and the reflection `r₁`, so `𝒢(βΦ)` is infinite.
The reflection `r₂` is *not* broken: every `μ_z` is invariant under it, and Georgii breaks it only
through a mixture, which is not formalised. **(6.21)(ii)** as well: each phase is invariant under the
subgroup generated by the vertical shift and the glide `t^{−z}∘θ_{(1,0)}`, the spin and lattice
reflections carry `μ_z` to `μ_{−z}`, and the mean is the staircase itself — which needed Example
(5.20)(1) generalised from the full shift group to any subgroup whose translates are Følner, and
the phase taken as an ultrafilter limit so the choice is equivariant.
...
Hasse graph of `ℤ` (12.11); (12.3)–(12.5) in full — the conditional independence across a bond
characterises Markov chains, Markov chains on trees are Markov fields, their
...
**Chapter 13, first steps** (`GibbsMeasure/Model/GaussianField.lean`): a Gauss field *is* Mathlib's
`IsGaussianProcess` for the coordinate process, with the Fourier characterisation (13.2) and (13.3),
