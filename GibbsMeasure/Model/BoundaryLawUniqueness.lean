...
theorem lazy_isTransferMatrix : IsTransferMatrix (Kernel.lazy P t) :=
  isTransferMatrix_of_stochastic (Kernel.lazy_pos hpos ht0) (Kernel.lazy_stochastic hP ht1.le)
...
theorem iInf_lazy_apply_self_pos :
    0 < ⨅ x, (Kernel.ofMatrix (Kernel.lazy P t) ^ 1) x {x} :=
  lt_of_lt_of_le (tsub_pos_of_lt ht1) (le_iInf fun x ↦ by
    rw [Kernel.ofMatrix_pow_one_apply_singleton]; exact Kernel.le_lazy_apply_self x)
...
theorem boundaryLawMeasure_const_lazy_mem_invariantG :
    boundaryLawMeasure (isBoundaryLaw_const (Kernel.lazy_stochastic hP ht1.le) hα0 hαt hα1
        (Kernel.tsum_mul_lazy hαP ht1.le))
      ∈ invariantG (transferSpecification (Kernel.lazy P t) (lazy_isTransferMatrix hpos hP ht0 ht1))
        (shiftGroup ℤ E) :=
  boundaryLawMeasure_const_mem_invariantG (Kernel.lazy_pos hpos ht0) (Kernel.lazy_stochastic hP ht1.le) hα0 hαt
    hα1 (Kernel.tsum_mul_lazy hαP ht1.le) (lazy_isTransferMatrix hpos hP ht0 ht1) rfl
...
The lazy matrix `Kernel.lazy P t = tP + (1-t)I` and its binomial formula
`Kernel.ofMatrix_lazy_pow_apply_singleton` live in
`GibbsMeasure/Mathlib/Probability/Kernel/CountableMatrix.lean`; the analytic facts about its
convergence norm — `Kernel.lazy_convergenceNorm_le` (`L(Q) ≤ tL(P) + (1-t)`),
`Kernel.one_le_lazy_convergenceNorm_of_convergenceNorm_eq_one`, and their combination
`Kernel.lazy_convergenceNorm_eq_one_of_convergenceNorm_eq_one` (`L(P) = 1 ⟹ L(Q) = 1`), genuine
generalisations of Vere-Jones' remark after (11.6) — in
`GibbsMeasure/Mathlib/Probability/Kernel/CountableMatrix/Recurrence.lean`. The last one is exactly
the hypothesis `isPositiveRecurrent_of_apply_eq_mul_div` (Recurrence.lean) needs to rule out `Q`
being equivalent to a positive recurrent matrix once `Q` itself is known not to be positive
recurrent; `isPositiveRecurrent_ofMatrix_of_isPositiveRecurrent_lazy` supplies that last fact (an
invariant probability measure of `Q = tP + (1-t)I` is already `P`-invariant, since `t ≠ 0`).
`eq_empty_G_lazy_of_not_isPositiveRecurrent` assembles all of this into **Georgii Comment
...
(11.18)(3) are proved. The general formula also reproves Comment (11.18)(1) directly (no need
for the separate `t = 1` degenerate check Georgii's own remark makes for finite `E`). -/

@[deprecated ProbabilityTheory.Kernel.ofMatrix_lazy_pow_apply_singleton (since := "2026-09-05")]
alias ofMatrix_lazy_pow_apply_singleton :=
  ProbabilityTheory.Kernel.ofMatrix_lazy_pow_apply_singleton

@[deprecated ProbabilityTheory.Kernel.lazy_convergenceNorm_eq_one_of_convergenceNorm_eq_one
  (since := "2026-09-05")]
alias lazy_convergenceNorm_eq_one_of_convergenceNorm_eq_one :=
  ProbabilityTheory.Kernel.lazy_convergenceNorm_eq_one_of_convergenceNorm_eq_one
...
theorem isPositiveRecurrent_ofMatrix_of_isPositiveRecurrent_lazy {E : Type*} [MeasurableSpace E]
    ...
    (hP : ∀ x, ∑' y, P x y = 1) (ht0 : t ≠ 0) (ht1 : t < 1)
    (hpos : ∀ x y, 0 < P x y)
    (hposrecQ : ProbabilityTheory.Kernel.IsPositiveRecurrent (Kernel.ofMatrix (Kernel.lazy P t))) :
    ProbabilityTheory.Kernel.IsPositiveRecurrent (Kernel.ofMatrix P) := by
  obtain ⟨hrecQ, μ, hμprob, hμinvQ⟩ := hposrecQ
  have hkey : ∀ b : E, ∑' a, μ {a} * P a b = μ {b} := by
    intro b
    have h1 : μ {b} = ∑' a, (Kernel.ofMatrix (Kernel.lazy P t)) a {b} * μ {a} :=
      hμinvQ.apply_singleton_eq_tsum b
    simp only [Kernel.ofMatrix_apply_singleton, Kernel.lazy] at h1
...
theorem eq_empty_G_lazy_of_not_isPositiveRecurrent {E : Type*} [MeasurableSpace E] [Countable E]
    ...
    G (transferSpecification (Kernel.lazy P t) (lazy_isTransferMatrix hpos hP ht0 ht1)) = ∅ := by
  have hQ := lazy_isTransferMatrix hpos hP ht0 ht1
  have hnotequiv : ¬ ∃ (R : E → E → ℝ≥0∞) (q : ℝ≥0∞) (r : E → ℝ≥0∞), 0 < q ∧ q ≠ ⊤ ∧
      (∀ x, 0 < r x) ∧ (∀ x, r x ≠ ⊤) ∧ (∀ x y, R x y = Kernel.lazy P t x y * r y / (q * r x)) ∧
    ...
    have hMQ : IsMarkovKernel (Kernel.ofMatrix (Kernel.lazy P t)) :=
      Kernel.isMarkovKernel_ofMatrix _ (Kernel.lazy_stochastic hP ht1.le)
    have hLQ1 : Kernel.convergenceNorm (Kernel.ofMatrix (Kernel.lazy P t)) x = 1 :=
      Kernel.lazy_convergenceNorm_eq_one_of_convergenceNorm_eq_one hpos hP ht0 ht1 x hL1
    have hRQ' : ∀ x y, (Kernel.ofMatrix R) x {y}
        = (Kernel.ofMatrix (Kernel.lazy P t)) x {y} * r y / (q * r x) := fun x y ↦ by
      rw [Kernel.ofMatrix_apply_singleton, Kernel.ofMatrix_apply_singleton]; exact hRQ x y
    have hposrecQ : Kernel.IsPositiveRecurrent (Kernel.ofMatrix (Kernel.lazy P t)) :=
