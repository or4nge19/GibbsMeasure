...
weights on `ex P`, `w_μ(ν(A) ≤ c) = μ(μ(A | 𝒜) ≤ c)` — and of Corollaries (7.28), (7.29) and the
discretisation step of (7.30) use nothing about `P` beyond `ex P = P ∩ P_𝒜` and measurability of
`P`; they are proved once, in the namespace `IsPAKernel`, and instantiated at `P = 𝓟_Θ`
(Theorem (14.10)) and `P = 𝒢_Θ(γ)` (Theorem (14.17), Corollaries (14.18), (14.25)). The
Chapter 7 statements in `ExtremeDecomposition.lean` and `ExtremeCorollaries.lean` are the
instance `P = 𝒢(γ)`, `𝒜 = 𝓣`.
...
/-! ### Barycentres of weights carried by the invariant probability measures -/
...
variable {Ω : Type*} [m₀ : MeasurableSpace Ω] {μ : Measure Ω} {𝒜 ℬ : MeasurableSpace Ω}
...
Georgii's Proposition (7.22) produces, from a `(P, 𝒜)`-kernel `π`, the unique representing
weight `w_μ = weight π μ` of every `μ ∈ P`. The statements of Theorem (7.26) — `w_μ` is carried by
...
(linear independence counts extreme points) and (7.30) (the discretisation step) use nothing
about `P` beyond `ex P = P ∩ P_𝒜` and measurability of `P`. They are proved here once, at that
generality, and instantiated at `P = 𝓟_Θ` (Theorem (14.10)) and `P = 𝒢_Θ(γ)` (Theorem (14.17),
Corollaries (14.18), (14.25)). -/
...
variable {Ω : Type*} {𝒜 : MeasurableSpace Ω} [m : MeasurableSpace Ω]
  [MeasurableSpace.CountablyGenerated Ω] {P : Set (Measure Ω)} {π : Kernel[𝒜, m] Ω Ω}
...
variable {Ω : Type*} [MeasurableSpace Ω] {𝒞 : Set (Set Ω)}
...
lemma convexCombosLimitGibbs_eq_convexCombos {S E : Type*} [MeasurableSpace E]
    (γ : Specification S E) : convexCombosLimitGibbs γ = convexCombos (limitGibbs γ) := rfl
