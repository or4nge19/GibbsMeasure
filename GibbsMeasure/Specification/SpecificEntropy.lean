...
* `relativeEntropyIn_coe_finset`, `relativeEntropyIn_map_shift`: in a *finite* volume the relative
  entropy is the relative entropy of the two marginals, `𝓗_Λ(μ | ν) = 𝓗(σ_Λ μ | σ_Λ ν)`
  (`InformationTheory.klDiv_trim_comap`); and `𝓗_Λ(θ_k μ | λ^S) = 𝓗_{Λ - k}(μ | λ^S)` for an
  arbitrary `μ` (only `λ^S` need be shift invariant).
* `relativeEntropyIn_finset_sum_smul_le`, `finset_sum_smul_relativeEntropyIn_le`: the `n`-ary
  forms of Georgii (15.5)(d) and of the convexity defect, i.e. Georgii's "obvious extension of
  Proposition (15.14)" for finite mixtures `∑ᵢ aᵢ μᵢ`.
...
* `map_finsetRestrict_eq_withDensity_uniformOn`, `trim_cylinderEvents_eq_withDensity_uniformOn`,
  `rnDeriv_trim_cylinderEvents_uniformOn_ae_eq`, **Georgii's finite-volume density**: over a
  finite state space with the uniform a priori measure, `f_Λ(ω) = |E|^{|Λ|} μ(σ_Λ = σ_Λ(ω))` is
  the density of `μ` on `𝓕_Λ` with respect to `λ^S`, in `withDensity` form and as a version of
  the Radon–Nikodym derivative. This is the `f_Λ` of the theorem of McMillan
  (`GibbsMeasure/Specification/ShannonMcMillan.lean`).
...
/-- **The relative entropy in a finite volume is the relative entropy of the two marginals**:
`𝓗_Λ(μ | ν) = 𝓗(σ_Λ μ | σ_Λ ν)`, because `𝓕_Λ` is the σ-algebra generated by the restriction
`σ_Λ` (`InformationTheory.klDiv_trim_comap`). -/
lemma relativeEntropyIn_coe_finset [IsFiniteMeasure μ] [IsFiniteMeasure ν] (Λ : Finset S) :
    relativeEntropyIn (Λ : Set S) μ ν = klDiv (μ.map Λ.restrict) (ν.map Λ.restrict) := by
  unfold relativeEntropyIn
  rw [klDiv_trim_congr (cylinderEvents_eq_comap_finsetRestrict (X := fun _ : S ↦ E) Λ)
      cylinderEvents_le_pi (Finset.measurable_restrict Λ).comap_le,
    klDiv_trim_comap (Finset.measurable_restrict Λ)]
...
/-! ### Finite mixtures (the `n`-ary form of Georgii (15.5)(d)) -/

section Mixture

variable {ι : Type*} {s : Finset ι}

/-- **Concavity of the entropy over a finite mixture**, the `n`-ary form of Georgii (15.5)(d)
(`smul_entropyIn_add_smul_entropyIn_le`): for weights summing to `1`,
`𝓗_Λ(∑ᵢ aᵢ μᵢ | ν) ≤ ∑ᵢ aᵢ 𝓗_Λ(μᵢ | ν)`. -/
theorem relativeEntropyIn_finset_sum_smul_le {a : ι → ℝ≥0} (ha : ∑ i ∈ s, a i = 1)
    (μ : ι → Measure (S → E)) [∀ i, IsFiniteMeasure (μ i)] (ν : Measure (S → E))
    [IsFiniteMeasure ν] (Λ : Set S) :
    relativeEntropyIn Λ (∑ i ∈ s, a i • μ i) ν
      ≤ ∑ i ∈ s, (a i : ℝ≥0∞) * relativeEntropyIn Λ (μ i) ν := by
  have h : (∑ i ∈ s, a i • μ i).trim (cylinderEvents_le_pi (X := fun _ : S ↦ E) (Δ := Λ))
      = ∑ i ∈ s, a i • ((μ i).trim cylinderEvents_le_pi) := by
    rw [Measure.trim_finset_sum]
    exact Finset.sum_congr rfl fun i _ ↦ Measure.trim_smul _ _
  unfold relativeEntropyIn
  rw [h]
  exact klDiv_finset_sum_smul_le ha _ _

/-- **The convexity defect of the entropy over a finite mixture**, Georgii's "obvious extension of
Proposition (15.14)" in the proof of (15.52) and the `n`-ary form of `entropyIn_smul_add_smul_le`:
for weights `aᵢ > 0` summing to `1`,
`∑ᵢ aᵢ 𝓗_Λ(μᵢ | ν) ≤ 𝓗_Λ(∑ᵢ aᵢ μᵢ | ν) + ∑ᵢ aᵢ log (1/aᵢ)`. -/
theorem finset_sum_smul_relativeEntropyIn_le {a : ι → ℝ≥0} (ha : ∀ i ∈ s, 0 < a i)
    (hsum : ∑ i ∈ s, a i = 1) (μ : ι → Measure (S → E)) [∀ i, IsProbabilityMeasure (μ i)]
    (ν : Measure (S → E)) [IsProbabilityMeasure ν] (Λ : Set S) :
    ∑ i ∈ s, (a i : ℝ≥0∞) * relativeEntropyIn Λ (μ i) ν
      ≤ relativeEntropyIn Λ (∑ i ∈ s, a i • μ i) ν
        + ∑ i ∈ s, ENNReal.ofReal (-((a i : ℝ) * log (a i))) := by
  have h : (∑ i ∈ s, a i • μ i).trim (cylinderEvents_le_pi (X := fun _ : S ↦ E) (Δ := Λ))
      = ∑ i ∈ s, a i • ((μ i).trim cylinderEvents_le_pi) := by
    rw [Measure.trim_finset_sum]
    exact Finset.sum_congr rfl fun i _ ↦ Measure.trim_smul _ _
  unfold relativeEntropyIn
  rw [h]
  exact finset_sum_smul_klDiv_le ha hsum
...
/-- **The relative entropy of a shifted measure**: `𝓗_Λ(θ_k μ | λ^S) = 𝓗_{Λ - k}(μ | λ^S)`. No
invariance of `μ` is needed, only that of the a priori product measure `λ^S`. -/
lemma relativeEntropyIn_map_shift (lam : Measure E) [IsProbabilityMeasure lam] [IsFiniteMeasure μ]
    (Λ : Set S) (k : S) :
    relativeEntropyIn Λ (μ.map (shift E k).toFun) (Measure.infinitePi fun _ : S ↦ lam)
      = relativeEntropyIn ((· - k) '' Λ) μ (Measure.infinitePi fun _ : S ↦ lam) := by
  have h := klDiv_trim_map_eq_klDiv_trim_comap (μ := μ) (ν := Measure.infinitePi fun _ : S ↦ lam)
  ...
  rw [(measurePreserving_shift_infinitePi lam k).map_eq] at h
...
/-! ### The entropy in a finite volume over a finite state space -/

section FiniteVolume

variable {S E : Type*} [MeasurableSpace E] [Fintype E] [Nonempty E] [MeasurableSingletonClass E]
  {μ : Measure (S → E)} [IsProbabilityMeasure μ]

/-- Over a finite state space and for the uniform a priori measure, the product measure gives
every finite-volume cylinder `{ω | ω_Λ = v}` the mass `|E|^{-|Λ|}`. -/
lemma measure_infinitePi_uniformOn_restrict_preimage (Λ : Finset S) (v : Λ → E) :
    (Measure.infinitePi fun _ : S ↦ uniformOn (Set.univ : Set E)) (Λ.restrict ⁻¹' {v})
      = ((Fintype.card E : ℝ≥0∞) ^ #Λ)⁻¹ := by
  classical
  rw [← Measure.map_apply (Finset.measurable_restrict Λ) (measurableSet_singleton v),
    Measure.infinitePi_map_restrict]
  have hpi : (Measure.pi fun _ : Λ ↦ uniformOn (Set.univ : Set E))
      = uniformOn (Set.univ : Set (Λ → E)) := by
    have h := uniformOn_pi (Ω := E) (ι := (Λ : Finset S)) (f := fun _ ↦ (Set.univ : Set E))
    rw [Set.pi_univ] at h
    exact h.symm
  rw [hpi, uniformOn_univ, Measure.count_singleton, Fintype.card_fun, Fintype.card_coe,
    Nat.cast_pow, one_div]

variable (μ) in
/-- **Georgii's finite-volume density `f_Λ`, marginal form.** Over a finite state space with the
uniform a priori measure, the law of `σ_Λ` under `μ` has density `v ↦ |E|^{|Λ|} μ(σ_Λ = v)` with
respect to `λ^Λ`. -/
theorem map_finsetRestrict_eq_withDensity_uniformOn (Λ : Finset S) :
    μ.map Λ.restrict
      = (Measure.pi fun _ : Λ ↦ uniformOn (Set.univ : Set E)).withDensity
          fun v ↦ (Fintype.card E : ℝ≥0∞) ^ #Λ * μ (Λ.restrict ⁻¹' {v}) := by
  classical
  have hcard : ((Fintype.card E : ℝ≥0∞) ^ #Λ) ≠ 0 :=
    pow_ne_zero _ (by exact_mod_cast Fintype.card_ne_zero)
  have hcard' : ((Fintype.card E : ℝ≥0∞) ^ #Λ) ≠ ∞ := by finiteness
  refine Measure.ext_of_singleton fun v ↦ ?_
  have hpi : (Measure.pi fun _ : Λ ↦ uniformOn (Set.univ : Set E)) {v}
      = ((Fintype.card E : ℝ≥0∞) ^ #Λ)⁻¹ := by
    rw [← Measure.infinitePi_map_restrict (μ := fun _ : S ↦ uniformOn (Set.univ : Set E)),
      Measure.map_apply (Finset.measurable_restrict Λ) (measurableSet_singleton v),
      measure_infinitePi_uniformOn_restrict_preimage Λ v]
  rw [Measure.map_apply (Finset.measurable_restrict Λ) (measurableSet_singleton v),
    withDensity_apply _ (measurableSet_singleton v), lintegral_singleton, hpi, mul_right_comm,
    ENNReal.mul_inv_cancel hcard hcard', one_mul]

variable (μ) in
/-- **Georgii's finite-volume density `f_Λ`.** Over a finite state space with the uniform a priori
measure `λ`, the restriction of `μ` to the finite-volume σ-algebra `𝓕_Λ` has, with respect to the
restriction of `λ^S`, the density
`f_Λ(ω) = |E|^{|Λ|} μ(σ_Λ = σ_Λ(ω))`,
which is the quantity appearing in
`MeasureTheory.GibbsMeasure.tendsto_integral_abs_inv_card_mul_log_density_add_specificEntropy`
(`GibbsMeasure/Specification/ShannonMcMillan.lean`). -/
theorem trim_cylinderEvents_eq_withDensity_uniformOn (Λ : Finset S) :
    μ.trim (cylinderEvents_le_pi (X := fun _ : S ↦ E) (Δ := (Λ : Set S)))
      = ((Measure.infinitePi fun _ : S ↦ uniformOn (Set.univ : Set E)).trim
            cylinderEvents_le_pi).withDensity
          fun ω ↦ (Fintype.card E : ℝ≥0∞) ^ #Λ * μ (Λ.restrict ⁻¹' {Λ.restrict ω}) := by
  classical
  set g : (Λ → E) → ℝ≥0∞ := fun v ↦ (Fintype.card E : ℝ≥0∞) ^ #Λ * μ (Λ.restrict ⁻¹' {v}) with hg
  have hgm : Measurable g := measurable_of_finite g
  have hres : Measurable[cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S)] Λ.restrict := by
    rw [cylinderEvents_eq_comap_finsetRestrict]
    exact Measurable.of_comap_le le_rfl
  refine @Measure.ext _ (cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S)) _ _ fun s hs ↦ ?_
  have hs' := hs
  rw [cylinderEvents_eq_comap_finsetRestrict] at hs'
  obtain ⟨B, hB, rfl⟩ := hs'
  have hind : ∀ ω : S → E, (Λ.restrict ⁻¹' B).indicator (fun ω ↦ g (Λ.restrict ω)) ω
      = (B.indicator g) (Λ.restrict ω) := by
    intro ω
    by_cases hω : Λ.restrict ω ∈ B <;>
      simp [Set.indicator_of_mem, Set.indicator_of_notMem, hω]
  have hmeas : Measurable[cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S)]
      fun ω ↦ (B.indicator g) (Λ.restrict ω) := (hgm.indicator hB).comp hres
  rw [trim_measurableSet_eq _ hs, withDensity_apply _ hs, ← lintegral_indicator hs,
    lintegral_congr hind, lintegral_trim _ hmeas,
    ← lintegral_map (hgm.indicator hB) (Finset.measurable_restrict Λ), lintegral_indicator hB,
    ← withDensity_apply _ hB, Measure.infinitePi_map_restrict,
    ← map_finsetRestrict_eq_withDensity_uniformOn μ Λ,
    Measure.map_apply (Finset.measurable_restrict Λ) hB]

variable (μ) in
/-- **Georgii's `f_Λ` is a Radon–Nikodym density.** Over a finite state space with the uniform a
priori measure, `f_Λ(ω) = |E|^{|Λ|} μ(σ_Λ = σ_Λ(ω))` is a version of the density
`dμ|𝓕_Λ / dλ^S|𝓕_Λ` of `μ` on the finite-volume σ-algebra. -/
theorem rnDeriv_trim_cylinderEvents_uniformOn_ae_eq (Λ : Finset S) :
    (μ.trim (cylinderEvents_le_pi (X := fun _ : S ↦ E) (Δ := (Λ : Set S)))).rnDeriv
        ((Measure.infinitePi fun _ : S ↦ uniformOn (Set.univ : Set E)).trim cylinderEvents_le_pi)
      =ᵐ[μ] fun ω ↦ (Fintype.card E : ℝ≥0∞) ^ #Λ * μ (Λ.restrict ⁻¹' {Λ.restrict ω}) := by
  classical
  set hm := cylinderEvents_le_pi (X := fun _ : S ↦ E) (Δ := (Λ : Set S)) with hmdef
  set lamS := Measure.infinitePi fun _ : S ↦ uniformOn (Set.univ : Set E) with hlamS
  set f : (S → E) → ℝ≥0∞ :=
    fun ω ↦ (Fintype.card E : ℝ≥0∞) ^ #Λ * μ (Λ.restrict ⁻¹' {Λ.restrict ω}) with hf
  have hres : Measurable[cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S)] Λ.restrict := by
    rw [cylinderEvents_eq_comap_finsetRestrict]
    exact Measurable.of_comap_le le_rfl
  have hfm : Measurable[cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S)] f :=
    (measurable_of_finite fun v : Λ → E ↦
      (Fintype.card E : ℝ≥0∞) ^ #Λ * μ (Λ.restrict ⁻¹' {v})).comp hres
  have hwd : μ.trim hm = (lamS.trim hm).withDensity f :=
    trim_cylinderEvents_eq_withDensity_uniformOn μ Λ
  have h := @Measure.rnDeriv_withDensity _ (cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S))
    (lamS.trim hm) _ f hfm
  rw [← hwd] at h
  refine ae_of_ae_trim hm (Measure.AbsolutelyContinuous.ae_le ?_ h)
  rw [hwd]
  exact withDensity_absolutelyContinuous _ _

omit [Fintype E] [Nonempty E] in
/-- Over a finite state space, the finite-volume σ-algebra `𝓕_Λ` is generated by the finitely
many cylinders `{ω | ω_Λ = v}`, `v ∈ E^Λ`. -/
lemma generateFrom_range_restrict_preimage [Finite E] (Λ : Finset S) :
    MeasurableSpace.generateFrom
        (Set.range fun v : Λ → E ↦ (Λ.restrict ⁻¹' {v} : Set (S → E)))
      = cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S) := by
  classical
  rw [cylinderEvents_eq_comap_finsetRestrict]
  refine le_antisymm (MeasurableSpace.generateFrom_le fun A hA ↦ ?_) fun A hA ↦ ?_
  · obtain ⟨v, rfl⟩ := hA
    exact ⟨{v}, measurableSet_singleton v, rfl⟩
  · obtain ⟨B, -, rfl⟩ := hA
    have hB : (Λ.restrict ⁻¹' B : Set (S → E))
        = ⋃ v ∈ B, (Λ.restrict ⁻¹' {v} : Set (S → E)) := by
      ext ω; simp
    rw [hB]
    exact MeasurableSet.biUnion B.to_countable fun v _ ↦
      MeasurableSpace.measurableSet_generateFrom ⟨v, rfl⟩

/-- **Shannon's formula in a finite volume.** For a finite state space `E` and the uniform a
priori measure `λ = |E|⁻¹ ∑_x δ_x`, the entropy of `μ` in a finite volume `Λ` is the Shannon
entropy `-∑_v μ(σ_Λ = v) log μ(σ_Λ = v)` of the distribution of `σ_Λ`, less the maximal entropy
`|Λ| log |E|`. Georgii takes `λ` to be counting measure, for which the constant is absent. -/
theorem entropyIn_uniformOn_eq_neg_integral_log_measureReal (Λ : Finset S) :
    entropyIn (uniformOn Set.univ) (Λ : Set S) μ
      = ((-(∫ ω, log (μ.real (Λ.restrict ⁻¹' {Λ.restrict ω} : Set (S → E))) ∂μ)
          - #Λ * log (Fintype.card E) : ℝ) : EReal) := by
  classical
  set ν : Measure (S → E) := Measure.infinitePi fun _ : S ↦ uniformOn (Set.univ : Set E) with hνdef
  set P : Finset (Set (S → E)) :=
    Finset.univ.image (fun v : Λ → E ↦ (Λ.restrict ⁻¹' {v} : Set (S → E))) with hPdef
  have hcard : (0 : ℝ) < ((Fintype.card E : ℝ) ^ #Λ)⁻¹ := by
    have h : (0 : ℝ) < Fintype.card E := by exact_mod_cast Fintype.card_pos
    positivity
  have hsurj : Function.Surjective (fun ω : S → E ↦ Λ.restrict ω) := fun v ↦
    ⟨fun i ↦ if h : i ∈ Λ then v ⟨i, h⟩ else Classical.arbitrary E, funext fun i ↦ by simp⟩
  have hinj : Function.Injective fun v : Λ → E ↦ (Λ.restrict ⁻¹' {v} : Set (S → E)) := by
    intro v w hvw
    have hvw' : (Λ.restrict ⁻¹' {v} : Set (S → E))
        = (Λ.restrict ⁻¹' {w} : Set (S → E)) := hvw
    obtain ⟨ω, hω⟩ := hsurj v
    have h1 : ω ∈ (Λ.restrict ⁻¹' {v} : Set (S → E)) := hω
    rw [hvw'] at h1
    exact hω.symm.trans h1
  have hmem : ∀ A ∈ P, ∃ v : Λ → E, A = (Λ.restrict ⁻¹' {v} : Set (S → E)) := by
    intro A hA
    simp only [hPdef, Finset.mem_image, Finset.mem_univ, true_and] at hA
    obtain ⟨v, rfl⟩ := hA
    exact ⟨v, rfl⟩
  have hP_meas : ∀ A ∈ P, MeasurableSet A := by
    intro A hA
    obtain ⟨v, rfl⟩ := hmem A hA
    exact (Finset.measurable_restrict Λ) (measurableSet_singleton v)
  have hP_disj : (P : Set (Set (S → E))).PairwiseDisjoint id := by
    intro A hA B hB hAB
    obtain ⟨v, rfl⟩ := hmem A hA
    obtain ⟨w, rfl⟩ := hmem B hB
    refine Set.disjoint_left.2 fun ω hωv hωw ↦ hAB ?_
    have h1 : Λ.restrict ω = v := hωv
    have h2 : Λ.restrict ω = w := hωw
    rw [← h1, ← h2]
  have hP_cover : ⋃₀ (P : Set (Set (S → E))) = Set.univ :=
    Set.eq_univ_of_forall fun ω ↦ Set.mem_sUnion.2
      ⟨(Λ.restrict ⁻¹' {Λ.restrict ω} : Set (S → E)), by simp [hPdef], rfl⟩
  have hPrange : (P : Set (Set (S → E)))
      = Set.range fun v : Λ → E ↦ (Λ.restrict ⁻¹' {v} : Set (S → E)) := by
    rw [hPdef, Finset.coe_image, Finset.coe_univ, Set.image_univ]
  have hgen : MeasurableSpace.generateFrom (P : Set (Set (S → E)))
      = cylinderEvents (X := fun _ : S ↦ E) (Λ : Set S) := by
    rw [hPrange]; exact generateFrom_range_restrict_preimage Λ
  have hνreal : ∀ v : Λ → E, ν.real (Λ.restrict ⁻¹' {v} : Set (S → E))
      = ((Fintype.card E : ℝ) ^ #Λ)⁻¹ := by
    intro v
    rw [measureReal_def, hνdef, measure_infinitePi_uniformOn_restrict_preimage Λ v,
      ENNReal.toReal_inv, ENNReal.toReal_pow, ENNReal.toReal_natCast]
  have hac : ∀ A ∈ P, ν A = 0 → μ A = 0 := by
    intro A hA h0
    obtain ⟨v, rfl⟩ := hmem A hA
    exact absurd ((measureReal_eq_zero_iff (measure_ne_top _ _)).2 h0)
      (by rw [hνreal v]; exact hcard.ne')
  have hkl := klDiv_trim_generateFrom_eq_sum (μ := μ) (ν := ν) hP_disj hP_cover hP_meas hac
  have hklR := toReal_klDiv_trim_generateFrom_eq_sum (μ := μ) (ν := ν) hP_disj hP_cover hP_meas hac
  have hcongr : klDiv (μ.trim (MeasurableSpace.generateFrom_le hP_meas))
      (ν.trim (MeasurableSpace.generateFrom_le hP_meas))
      = relativeEntropyIn (Λ : Set S) μ ν := klDiv_trim_congr hgen _ _
  have hfin : relativeEntropyIn (Λ : Set S) μ ν ≠ ⊤ := by
    rw [← hcongr, hkl]
    exact (ENNReal.sum_lt_top.2 fun A _ ↦ ENNReal.ofReal_lt_top).ne
  have hsum : ∑ A ∈ P, μ.real A * log (μ.real A / ν.real A)
      = (∫ ω, log (μ.real (Λ.restrict ⁻¹' {Λ.restrict ω} : Set (S → E))) ∂μ)
        + #Λ * log (Fintype.card E) := by
    rw [hPdef, Finset.sum_image fun v _ w _ h ↦ hinj h]
    have hterm : ∀ v : Λ → E,
        μ.real (Λ.restrict ⁻¹' {v} : Set (S → E))
            * log (μ.real (Λ.restrict ⁻¹' {v} : Set (S → E))
              / ν.real (Λ.restrict ⁻¹' {v} : Set (S → E)))
          = μ.real (Λ.restrict ⁻¹' {v} : Set (S → E))
              * log (μ.real (Λ.restrict ⁻¹' {v} : Set (S → E)))
            + μ.real (Λ.restrict ⁻¹' {v} : Set (S → E)) * (#Λ * log (Fintype.card E)) := by
      intro v
      rcases eq_or_ne (μ.real (Λ.restrict ⁻¹' {v} : Set (S → E))) 0 with h0 | h0
      · rw [h0]; ring
      · rw [hνreal v, log_div h0 hcard.ne', log_inv, log_pow]
        ring
    simp only [hterm]
    rw [Finset.sum_add_distrib, ← Finset.sum_mul]
    have h1 : ∑ v : Λ → E, μ.real (Λ.restrict ⁻¹' {v} : Set (S → E)) = 1 := by
      have h := sum_measureReal_preimage_singleton (μ := μ) (Finset.univ : Finset (Λ → E))
        (f := fun ω : S → E ↦ Λ.restrict ω)
        fun v _ ↦ (Finset.measurable_restrict Λ) (measurableSet_singleton v)
      rw [h]
      simp
    have h2 : ∑ v : Λ → E, μ.real (Λ.restrict ⁻¹' {v} : Set (S → E))
          * log (μ.real (Λ.restrict ⁻¹' {v} : Set (S → E)))
        = ∫ ω, log (μ.real (Λ.restrict ⁻¹' {Λ.restrict ω} : Set (S → E))) ∂μ := by
      have hFm : Measurable
          fun v : Λ → E ↦ log (μ.real (Λ.restrict ⁻¹' {v} : Set (S → E))) :=
        measurable_of_countable _
      have hprob : IsProbabilityMeasure (μ.map fun ω : S → E ↦ Λ.restrict ω) :=
        Measure.isProbabilityMeasure_map (Finset.measurable_restrict Λ).aemeasurable
      have hmapint : ∫ v, log (μ.real (Λ.restrict ⁻¹' {v} : Set (S → E)))
            ∂(μ.map fun ω : S → E ↦ Λ.restrict ω)
          = ∫ ω, log (μ.real (Λ.restrict ⁻¹' {Λ.restrict ω} : Set (S → E))) ∂μ :=
        integral_map (Finset.measurable_restrict Λ).aemeasurable hFm.aestronglyMeasurable
      rw [← hmapint, integral_countable Integrable.of_finite, tsum_fintype]
      refine Finset.sum_congr rfl fun v _ ↦ ?_
      have hmapv : (μ.map fun ω : S → E ↦ Λ.restrict ω).real {v}
          = μ.real (Λ.restrict ⁻¹' {v} : Set (S → E)) := by
        rw [measureReal_def, measureReal_def]
        congr 1
        exact Measure.map_apply (Finset.measurable_restrict Λ) (measurableSet_singleton v)
      rw [hmapv, smul_eq_mul]
    rw [h1, h2, one_mul]
  have hνuniv : ν.real Set.univ = 1 := by rw [hνdef]; exact probReal_univ
  have hval : (relativeEntropyIn (Λ : Set S) μ ν).toReal
      = (∫ ω, log (μ.real (Λ.restrict ⁻¹' {Λ.restrict ω} : Set (S → E))) ∂μ)
        + #Λ * log (Fintype.card E) := by
    rw [← hcongr, hklR, hsum, hνuniv, probReal_univ]
    ring
  rw [entropyIn, ← hνdef, ← EReal.coe_ennreal_toReal hfin, hval, ← EReal.coe_neg]
  congr 1
  ring
