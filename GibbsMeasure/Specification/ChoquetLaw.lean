...
lemma measurableSet_tailPiNatGen_tail (t : Finset ℕ) :
  ...
  let : MeasurableSpace Ω := (@tailSigmaAlgebra S E _)
  have : MeasurableSpace.CountablyGenerated Ω := by
    rcases ( (inferInstance :
        MeasurableSpace.CountableOrCountablyGenerated Ω Ω).countableOrCountablyGenerated ) with
            hΩ | hcg
    · have : Countable Ω := hΩ
...
lemma isTailTrivial_of_isTailTrivialCore {ν : Measure Ω}
  ...
  have : IsProbabilityMeasure νt := ⟨hνt_univ⟩
...
theorem isGibbsMeasure_of_isGibbsCore {μ : Measure Ω}
  ...
  have : IsProbabilityMeasure μ := ⟨hμ_univ⟩
  have hfix : ∀ Λ : Finset S, μ.bind (γ Λ) = μ := by
    ...
    have hγmeas : Measurable fun ω : Ω => (γ Λ ω : Measure Ω) := by
      exact (Kernel.measurable (γ Λ)).mono
        (MeasureTheory.cylinderEvents_le_pi (X := fun _ : S ↦ E) (Δ := ((Λ : Set S)ᶜ))) le_rfl
    have : IsProbabilityMeasure (μ.bind (γ Λ)) := by
...
theorem ae_mem_extremePoints_G_tailKernelLaw
  ...
  have hae_trim :
    ...
    have hGcore :
      ...
      have : IsProbabilityMeasure (tailKernel (S := S) (E := E) (μ := μ) ω) := by infer_instance
...
theorem tailKernelLaw_goodSet_eq_one
  ...
  have : IsProbabilityMeasure (tailKernelLaw (S := S) (E := E) (μ := μ)) :=
...
theorem ae_tailKernel_eq_of_isTailTrivial
  ...
  have hA_eq (A : Set Ω) (hA : MeasurableSet A) :
    ...
    have hgc_const : ∃ c : ℝ≥0∞, g =ᵐ[μt] fun _ => c := by
      have : IsProbabilityMeasure μt := ⟨hμt_univ⟩
      have hDich :
  ...
  have hae_eq :
    ...
    have hμν : ∀ s ∈ C,
      ...
      rcases hs with ⟨t, rfl⟩
      simpa using hω t
    have : IsProbabilityMeasure (tailKernel (S := S) (E := E) (μ := μ) ω) :=
