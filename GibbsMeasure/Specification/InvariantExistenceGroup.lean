...
* `MeasureTheory.GibbsMeasure.exists_mem_GP_forall_measurePreserving_shift_of_isCompact`:
  **Georgii Example (5.17)(1)** — Corollary (5.16) for the shift group `Θ` on an abelian site
  group, with no hypothesis on the state space — together with the two-group forms
  `exists_mem_GP_forall_measurePreserving_shift_and_measurePreserving_of_isCompact` and
  `..._and_involution_of_isCompact` (`I₀ = Θ`, `I₁` abelian resp. generated by an involution),
  and `exists_isGibbsMeasure_shift_and_siteEquiv_invariant`, **Example (5.17)(2)** for a finite
  group of site automorphisms;
* `Potential.invariantG_gibbsSpecification_shiftGroup_nonempty`, **Theorem (4.23)(a) with
  Corollary (5.16)**: `𝒢_Θ(γ^Φ) ≠ ∅` for a shift-invariant `Φ ∈ ℬ` over a standard Borel state
  space.
...
/-! ### Georgii Example (5.17)(1): the shift group

On an abelian group of sites the shift group `Θ` is abelian, so Corollary (5.16) applies to it
with no hypothesis on the state space; Theorem (5.15)(ii) then adds a further abelian group of
symmetries commuting with `Θ`, the shape of Georgii's Examples (5.17)(2)–(4). -/
...
(`shift_mul_shift`). No hypothesis on the state space is needed. -/
theorem exists_mem_GP_forall_measurePreserving_shift_of_isCompact {γ : Specification S E}
  ...
  exists_mem_GP_and_forall_measurePreserving_of_isCompact (Φ := shift E)
    (fun j j' ↦ (shift_mul_shift j j').symm) hγ hcpt hne
...
Georgii's site set is `ℤ^d`; nothing beyond `[Countable S]` and `[AddCommGroup S]` is used.
...
(`Potential.isInvariant_gibbsSpecification`) makes `γ^Φ` shift invariant, and Example (5.17)(1)
(`MeasureTheory.GibbsMeasure.exists_mem_GP_forall_measurePreserving_shift_of_isCompact`, itself
Corollary (5.16) for the abelian shift group) averages a Gibbs measure over Følner sets of shifts
into a shift-invariant one. -/
theorem invariantG_gibbsSpecification_shiftGroup_nonempty (hΦ : Φ.IsShiftInvariant) :
  ...
  obtain ⟨μ, hμ, hinv⟩ :=
    exists_mem_GP_forall_measurePreserving_shift_of_isCompact
      (γ := gibbsSpecificationOfAbsolutelySummable (Φ := Φ) ν β)
