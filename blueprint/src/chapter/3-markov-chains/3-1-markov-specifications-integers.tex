...
\begin{theorem}[Georgii, step 4 of the proof of (3.5): equation (3.12)]
    \label{thm:mkv-312}
    \uses{def:mkv-pos-hom-markov, def:proper-spec}
...
\begin{theorem}[Georgii (3.6)]
    ...
    \lean{MeasureTheory.GibbsMeasure.Markov.isCondExp_markovSpecification_stationaryChain}
    \leanok

    For every $\Lambda \in \Fin$, $\gamma_{P,\Lambda}$ is a version of the conditional
    distribution of $\mu_P$ given the configuration outside $\Lambda$.

    Georgii states (3.6) as $\gamma_\Lambda(\sigma_\Lambda = \zeta \mid \omega) =
    \mu_P(\sigma_\Lambda = \zeta \mid \sigma_{\partial\Lambda} = \omega_{\partial\Lambda})$,
    i.e.\ conditioning on the \emph{boundary} $\partial\Lambda$ alone. The Lean statement is at
    different hypotheses: it conditions on the whole exterior $\sigma$-algebra
    $\mathcal{F}_{\mathbb{Z}\setminus\Lambda}$. The extra content of Georgii's version --- that
    the kernel depends on $\omega$ only through $\omega_{\partial\Lambda}$ --- is available for
    interval volumes as Theorem \ref{thm:mkv-38-1-single}, where only $\omega_{a-1}$ and
    $\omega_{b+1}$ occur, and for singletons as (3.11); it is not stated for a general finite
    $\Lambda$.
\end{theorem}
\begin{proof}
    \leanok

    Unfolding the definition of a Gibbs measure, this is Theorem \ref{thm:mkv-step1}.
\end{proof}
