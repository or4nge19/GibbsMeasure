...
Throughout this section the parameter set is $S = \mathbb{Z}$, the state space $E$ is a finite
non-empty set with $\mathcal{E}$ its power set, and the a priori measure $\lambda$ is the uniform
probability measure on $E$; Georgii takes counting measure instead, and by Remark (1.28)(3) the
two choices have the same modifications and hence the same Gibbs measures. For
$\Lambda \subseteq \mathbb{Z}$,
\[
...
\begin{remark}[Comment (3.2): the determining function]
    ...
    Two specifications with equal singleton kernels agree, provided those kernels are densities,
    nowhere $0$ or $\infty$, against a disjointly consistent $\lambda$-independent family
    (Theorem (1.33)); this applies to the specifications of this section. Hence a positive
    homogeneous Markov specification is determined by the $g$ of Definition
    \ref{def:mkv-pos-hom-markov}, which is called its \textbf{determining function}.
\end{remark}
...
\begin{definition}[The homogeneous nearest-neighbour potential of a matrix, Georgii (3.9)]
    ...
    \[
        ...
        \end{cases}
    \]
    This is a homogeneous nearest-neighbour potential (with $\varphi_1 = 0$ and
    $\varphi_2 = -\log P$): each $\Phi^P_A$ is $\mathcal{F}_A$-measurable, the interactions
    containing $i$ lie inside $[i-1, i+1]$, so $\Phi^P$ has finite range, and
    $|\Phi^P_A| \le \sum_{x,y}|\log P(x,y)|$ uniformly, so with only three sets $A \ni i$
    contributing $\Phi^P$ is absolutely summable, $\Phi^P \in \Bspace$.
\end{definition}
...
\begin{definition}[The Markov specification of a matrix, Georgii (3.6)]
    ...
    $e^{-H_\Lambda^{\Phi^P}} / Z_\Lambda$. In particular $\gamma_P$ is proper and consistent.
\end{definition}
...
\begin{definition}[The stationary Markov chain $\mu_P$, Georgii (3.3)]
    ...
    \]
    where $\alpha_P$ is the stationary vector of Lemma \ref{lem:mkv-doeblin}. These interval
    weights form a projective family --- the right endpoint is dropped using
    $\sum_y P(x,y) = 1$, the left endpoint using $\alpha_P P = \alpha_P$ --- and $\mu_P$ is their
    projective limit by the Kolmogorov extension theorem.
\end{definition}
...
\begin{theorem}[Georgii, step 3 of the proof of (3.5): (3.12) $\Rightarrow$ (3.11)]
    ...
    $Q = (g(a,x,y)/g(a,a,y))_{x,y}$. Then for every $q > 0$ and every $v \in \,]0,\infty[^E$ ---
    no eigenvector equation assumed --- the matrix $M(x,y) = Q(x,y)v(y)/(q\,v(x))$ has $g$ as its
    determining function (3.11): $g(x,y,z) = M(x,y)M(y,z)/M^2(x,z)$. Taking for $(q,v)$ the
    Perron pair of $Q$ gives the matrix $P$ of (3.7).
\end{theorem}
\begin{proof}
    \leanok
...
\begin{proof}
    ...
    $\gamma_P$ have the same singleton kernels, and Remark \ref{rmk:mkv-comment-32} gives
    $\gamma = \gamma_P$.
\end{proof}
...
\begin{proof}
    ...
    move with $k$, so the ratio is estimated rather than the individual limits: since
...
\begin{theorem}[Georgii (3.6)]
    ...
    For every $\Lambda \in \Fin$, every $\zeta$ and every --- not merely almost every ---
    ...
    the conditioning on the finite boundary $\partial\Lambda$ of (3.4) being on an event of
    positive $\mu_P$-measure, so that the right-hand side is an elementary conditional
    probability. Moreover $\gamma_{P,\Lambda}$ is a version of the conditional distribution of
    $\mu_P$ given the whole exterior $\sigma$-algebra $\mathcal{F}_{\mathbb{Z}\setminus\Lambda}$.
\end{theorem}
\begin{proof}
    \leanok
    ...
    identity for $\mu_P$, inducting over the sites of $\Lambda$. The version statement for the
    exterior $\sigma$-algebra is Theorem \ref{thm:mkv-step1}.
\end{proof}
...
\begin{proof}
    ...
    function (3.11) (Theorem \ref{thm:mkv-311}). Step 2: $P$ is recovered from $g_P$ by (3.7),
    whence injectivity (Theorem \ref{thm:mkv-step2}, Corollary \ref{cor:mkv-inj}). Steps 3 and 4:
    ...
    the ergodic theorem for Markov chains gives $\Gibbs{\gamma_P} \subseteq \set{\mu_P}$
    (Theorem \ref{thm:mkv-uniqueness}).
\end{proof}
...
\begin{corollary}[Georgii (3.9)]
    ...
    \varphi_2(\sigma_i,\sigma_{i+1})$; in the forward direction $\gamma = \gamma_P$ is Gibbsian
    for $\Phi^P = -\log P$ of Definition \ref{def:mkv-potential} by Theorem \ref{thm:mkv-surj}.
    Consequently, by (2.35)(b), for a fixed $\alpha \in \Prob(E,\mathcal{E})$ the relation
...
\begin{example}[Georgii (3.13)--(3.19): the one-dimensional Ising model]
    ...
    Take $S = \mathbb{Z}$ with the nearest-neighbour graph, $E = \set{\pm 1}$, and the Ising
    potential $\Phi^{J,h}$ of (3.13): $\Phi_{\set i} = -h\sigma_i$,
    $\Phi_{\set{i,i+1}} = -J\sigma_i\sigma_{i+1}$, which is the homogeneous nearest-neighbour
    potential of $\varphi_1 = -h\sigma$, $\varphi_2 = -J\sigma\sigma$. Its determining function
    is (3.14); the associated matrix $Q$ has Perron--Frobenius eigenvalue
    $q_{J,h} = e^{-h}\bigl(\cosh h + \sqrt{e^{-4J} + \sinh^2 h}\bigr)$ (3.16); formula (3.7)
    produces the transition matrix $P_{J,h}$ of (3.17); and therefore, by Theorem
    \ref{thm:mkv-35}, $\Gibbs{\beta\Phi^{J,h}} = \set{\mu_{\beta J,\beta h}}$ is a singleton
    (3.15) --- no phase transition in one dimension at any temperature. The stationary
    distribution is (3.18) and the magnetisation is
...
\begin{remark}[The half-line variant]
    ...
    Theorem (3.5) has a straightforward modification for $S = \mathbb{N}$, with $\mu_P$ replaced
    by the Markov chain with transition matrix $P$ started at time $0$ in a fixed state
    $a \in E$.
\end{remark}
