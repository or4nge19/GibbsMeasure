...
\begin{proposition}[The deterministic case is Mathlib's ergodicity]
    ...
    invariant set to a strictly invariant one. Example \ref{ex:ext-75} shows that no such
    correction is available for general kernels, so the passage from strict to almost sure
    invariance is not a mere technical convenience.
\end{proposition}
