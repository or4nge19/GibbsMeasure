...
\begin{proof}
    ...
    Properness of $\gamma_\Lambda$ is properness of a parallel composition of kernels, and
    consistency follows from Fubini for parallel binds: pushing a product measure through
    $\gamma_\Lambda$ binds the two factors separately, so
    $(\mu^1 \times \mu^2)\gamma_\Lambda
      = \mu^1\gamma^1_{\Lambda \inter S_1} \times \mu^2\gamma^2_{\Lambda \inter S_2}$. Taking
    $\mu^k = \gamma^k_{\Lambda'_k}(\,\cdot \mid \omega^k)$ gives consistency, and taking
...
\begin{lemma}[Zero-one laws multiply]
    ...
    If $\mu$ is trivial on $\bigcap_i m^1_i$ and $\nu$ on $\bigcap_j m^2_j$, then
    $\mu \otimes \nu$ is trivial on $\bigcap_i \bigcap_j m^1_i \otimes m^2_j$.  The double
    intersection is essential: it contains
    $(\bigcap_i m^1_i) \otimes (\bigcap_j m^2_j)$ strictly in general, and it is the double
    intersection, not the product of the tails, that contains the tail
    $\sigma$-algebra of $E^{S_1 \sqcup S_2}$.
\end{lemma}
\begin{proof}
    \leanok
