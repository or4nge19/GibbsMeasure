...
$\Tail = \bigcap_i \mathcal{T}_i$, and $\gamma$ by any family of proper probability
...
\begin{example}[Georgii (7.14), Kolmogorov's zero-one law]
    ...
    Formalised in \texttt{GibbsMeasure/Specification/KolmogorovZeroOne.lean}: the zero-one law
    for $\prod_i \alpha_i$ (\texttt{forall\_tail\_measure\_eq\_zero\_or\_one\_infinitePi}, with
    \emph{no} countability assumption on $S$ --- the tail of an uncountable product is handled by
    the same cofinite-limsup description of $\Tail$), its packaging as tail triviality
    (\texttt{isTailTrivial\_infinitePi}), and Georgii's route as stated:
    $\Gibbs{\ISSSD(\lambda)} = \set{\lambda^S}$ as a set identity
    (\texttt{G\_isssd\_eq\_singleton}) and the resulting extremality
    (\texttt{infinitePi\_mem\_extremePoints\_G\_isssd}).  The zero-one law is proved directly from
    Mathlib's filter-indexed Kolmogorov law rather than through
    Theorem \ref{thm:app-extreme-iff-tail-trivial}, which is why no countability of $S$ enters;
    the inhomogeneous case $\prod_i \alpha_i$ comes with it.
\end{example}
...
\begin{example}[Georgii (7.15), stationary Markov chains]
    ...
    Formalised in \texttt{GibbsMeasure/Model/MarkovChain.lean}: extremality of $\mu_P$ in
    $\Gibbs{\gamma_P}$ from the Theorem (3.5) singleton
    (\texttt{stationaryChain\_mem\_extremePoints\_G}) and its tail triviality via
    Theorem \ref{thm:app-extreme-iff-tail-trivial}
    (\texttt{forall\_tail\_stationaryChain\_eq\_zero\_or\_one}); triviality on the right-sided
    tail is the restriction of the latter to
    $\bigcap_n \mathcal{F}_{\mathbb{Z} \cap [n,\infty[} \subseteq \Tail$.  The convergence
    $|P^n(x,y) - \alpha(y)| \to 0$ via (7.9)/(7.11)(b) is not yet derived from it.
\end{example}
