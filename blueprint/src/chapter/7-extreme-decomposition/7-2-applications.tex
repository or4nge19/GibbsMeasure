...
Georgii's Remark (7.13): Theorems (7.7) and (7.12)(a) hold when the finite volumes are replaced
by a countable directed index set $\iota$, the σ-algebras $\mathcal{F}_{S \setminus \Lambda}$ by
a decreasing family $(\mathcal{T}_i)_{i \in \iota}$ with $\Tail = \bigcap_i \mathcal{T}_i$, and
$\gamma$ by proper probability kernels $\gamma_i$ from $\mathcal{T}_i$ to $\mathcal{F}$, with
$\Gibbs{\gamma}$ replaced by the convex set $\Prob_\gamma$ of $\gamma$-invariant probability
measures.  This section applies it to Kolmogorov's zero-one law (7.14), stationary Markov chains
(7.15), exchangeability and the Hewitt--Savage zero-one law (7.16)--(7.17), and product
specifications (7.18)--(7.19).
...
\begin{definition}[Abstract specification, Georgii, Remark (7.13)]
    ...
    or directedness assumption.  An \textbf{abstract specification} is a decreasing family
    $(\mathcal{T}_i)_{i \in \iota}$ of sub-σ-algebras of $\mathcal{F}$ together with proper
    probability kernels $\gamma_i$ from $(\Cfg, \mathcal{T}_i)$ to $(\Cfg, \mathcal{F})$
...
\begin{lemma}[Specifications are abstract specifications]
    ...
    $\Cfg = E^S$ and its invariant set is $\Gibbs{\gamma}$.
\end{lemma}
\begin{proof}
    \leanok

    Consistency, properness and the Markov property are the defining data of a specification;
    $\Lambda \mapsto \mathcal{F}_{S\setminus\Lambda}$ is decreasing.  The tail σ-algebra is
    $\bigwedge_\Lambda \mathcal{F}_{S\setminus\Lambda}$ by definition, and
...
\begin{proof}
    ...
    If $A \in \Tail$ has $0 < \mu(A) < 1$, properness pulls $A$ out of $\mu \gamma_i$, so
    $\mu(\cdot \mid A)$ and $\mu(\cdot \mid A^{c})$ are $\gamma$-invariant (Georgii (7.7)(b))
    and $\mu$ is their proper convex combination.  Conversely, let $\mu$ be trivial on $\Tail$
    and $\nu \in \Prob_\gamma$ with $\nu \ll \mu$.  For each $i$ the density
    $\mathrm{d}\nu/\mathrm{d}\mu$ has a $\mathcal{T}_i$-measurable version, since passing to a
    density commutes with $\mu \mapsto \mu\gamma_i$ for proper $\gamma_i$; a $\limsup$ along a
    cofinal sequence gives a $\Tail$-measurable version, a.e.\ constant by tail triviality, so
    $\nu = \mu$.  Both parts of a proper convex decomposition of $\mu$ are absolutely continuous
    with respect to $\mu$, so $\mu$ is extreme.
\end{proof}
...
\begin{proposition}[Georgii (7.25), in the generality of Remark (7.13)]
    ...
    $\Prob_\gamma \neq \emptyset$ there is a probability kernel $\pi$ from $(\Cfg, \Tail)$ to
    $(\Cfg, \mathcal{F})$ which is a version of $\mu(\cdot \mid \Tail)$ simultaneously for
    every $\mu \in \Prob_\gamma$ and takes all its values in $\Prob_\gamma$: a
    $(\Prob_\gamma, \Tail)$-kernel in the sense of Georgii's Definition (7.21).
\end{proposition}
\begin{proof}
    \leanok

    Along a cofinal sequence $(i_n)$, Lévy's downward theorem and the invariance equation
    identify $\lim_n \gamma_{i_n}(A \mid \cdot)$ with $\mu(A \mid \Tail)$ for every
    $\mu \in \Prob_\gamma$; the limit does not depend on $\mu$.  Applied to the half-lines
    $\set{e \le q}$, $q \in \mathbb{Q}$, for a Borel embedding $e$ of $\Cfg$ into $\R$, this
    gives a $\Tail$-measurable rational CDF, hence a kernel to $\R$ by the Stieltjes
    construction.  Pull it back along $e$ and replace its value by a fixed
    $\nu_0 \in \Prob_\gamma$ on the two tail events where the kernel is not carried by the range
    of $e$ or its value is not $\gamma$-invariant; both events are $\Tail$-measurable since
...
\begin{example}[Georgii (7.14), Kolmogorov's zero-one law]
    ...
    Moreover $\mu$ is the unique element of $\Gibbs{\gamma}$ for
    \[\gamma_\Lambda f(\omega) = \int \Big(\prod_{i \in \Lambda} \alpha_i\Big)(\mathrm{d}\zeta)\,
      f(\zeta\omega_{S \setminus \Lambda}),\]
    and for constant $\alpha_i = \lambda$ this reads $\Gibbs{\ISSSD(\lambda)} = \set{\lambda^S}$
    (Remark (1.25)); both are instances of Theorem \ref{thm:free-measure-gibbs}.  Hence $\mu$
    is extreme in $\Gibbs{\gamma}$ and trivial on $\Tail$ by
    Theorem \ref{thm:app-extreme-iff-tail-trivial}.
\end{example}

\begin{example}[Georgii (7.18), product specifications]
    \label{ex:app-product}
    \uses{def:spec, def:gibbs-meas}
    \lean{Specification.prod, Specification.prod_apply,
      Specification.isGibbsMeasure_prod_map}
    \leanok

    Let $\gamma^1, \gamma^2$ be specifications with the same state space $(E,\mathcal E)$ and
    parameter sets $S_1, S_2$. On the disjoint union $S_1 \sqcup S_2$ the kernels
    \[\gamma_{\Lambda}(\,\cdot \mid \omega^1\omega^2)
      = \gamma^1_{\Lambda \inter S_1}(\,\cdot \mid \omega^1) \times
        \gamma^2_{\Lambda \inter S_2}(\,\cdot \mid \omega^2)\]
    form a specification $\gamma = \gamma^1 \times \gamma^2$, and
    \[\set{\mu^1 \times \mu^2 : \mu^k \in \Gibbs{\gamma^k}} \subseteq \Gibbs{\gamma},\]
    with strict inclusion as soon as both $\Gibbs{\gamma^k}$ have more than one element, since
    $(\mu^1 \times \mu^2 + \nu^1 \times \nu^2)/2$ is then Gibbs without being a product.
\end{example}
\begin{proof}
    \leanok

    Properness of $\gamma_\Lambda$ is properness of a parallel composition of kernels.  By
    Fubini for parallel binds,
    $(\mu^1 \times \mu^2)\gamma_\Lambda
      = \mu^1\gamma^1_{\Lambda \inter S_1} \times \mu^2\gamma^2_{\Lambda \inter S_2}$;
    $\mu^k = \gamma^k_{\Lambda'_k}(\,\cdot \mid \omega^k)$ gives consistency and
    $\mu^k \in \Gibbs{\gamma^k}$ gives the inclusion.
\end{proof}

\begin{theorem}[Georgii, equation (7.19)]
    \label{thm:app-product-extreme}
    \uses{ex:app-product, thm:app-extreme-iff-tail-trivial}
    \lean{MeasureTheory.GibbsMeasure.extremePoints_G_prod,
      MeasureTheory.GibbsMeasure.mem_extremePoints_G_prod_map,
      MeasureTheory.GibbsMeasure.exists_eq_map_prod_of_mem_extremePoints_G_prod,
      MeasureTheory.GibbsMeasure.mem_extremePoints_G_of_mem_extremePoints_G_prod_map,
      MeasureTheory.GibbsMeasure.bijective_prod_extremePoints_G,
      MeasureTheory.GibbsMeasure.card_extremePoints_G_prod}
    \leanok

    For the product specification $\gamma = \gamma^1 \times \gamma^2$ over countable $S_1, S_2$,
    \[\mathrm{ex}\,\Gibbs{\gamma} = \set{\mu^1 \times \mu^2 :
      \mu^k \in \mathrm{ex}\,\Gibbs{\gamma^k},\ k = 1,2},\]
    the map $(\mu^1,\mu^2) \mapsto \mu^1 \times \mu^2$ being a bijection; in particular
    $|\mathrm{ex}\,\Gibbs{\gamma}| = |\mathrm{ex}\,\Gibbs{\gamma^1}|\,
    |\mathrm{ex}\,\Gibbs{\gamma^2}|$, so an iterated product of the specifications of
    Section 6.1 has a prescribed power of two as its number of phases.
\end{theorem}
\begin{proof}
    \uses{lem:app-prod-zero-one}
    \leanok

    A product of measures trivial on $\Tail^1$, $\Tail^2$ is trivial on the tail of the product
    (Lemma \ref{lem:app-prod-zero-one}), so by Theorem \ref{thm:app-extreme-iff-tail-trivial} a
    product of extreme Gibbs measures is extreme. Conversely an extreme $\mu \in \Gibbs{\gamma}$
    is tail trivial, hence its $\mathcal{F}^1 \times \mathcal{F}^2$-sections factorise and
    $\mu = \mu^1 \times \mu^2$ with $\mu^k$ the marginals; each factor is extreme, since a
    non-trivial splitting of $\mu^1$ would split $\mu$.
\end{proof}

\begin{lemma}[Zero-one laws multiply]
    \label{lem:app-prod-zero-one}
    \lean{MeasureTheory.Measure.prod_apply_eq_zero_or_one_iInf,
      MeasureTheory.Measure.prod_apply_eq_zero_or_one,
      MeasureTheory.GibbsMeasure.isTailTrivial_map_symm_prod}
    \leanok

    Let $(m^1_i)_{i \in I}$, $(m^2_j)_{j \in J}$ be nonempty families of sub-σ-algebras of
    $\mathcal{F}^1$, $\mathcal{F}^2$ and $\mu$, $\nu$ probability measures on
    $\mathcal{F}^1$, $\mathcal{F}^2$.  If $\mu$ is trivial on $\bigcap_i m^1_i$ and $\nu$ on
    $\bigcap_j m^2_j$, then $\mu \otimes \nu$ is trivial on
    $\bigcap_i \bigcap_j m^1_i \otimes m^2_j$.  This double intersection contains
    $(\bigcap_i m^1_i) \otimes (\bigcap_j m^2_j)$, strictly in general, and it contains the
    tail σ-algebra of $E^{S_1 \sqcup S_2}$ when $m^k_\Lambda = \mathcal{F}_{S_k \setminus \Lambda}$.
\end{lemma}
\begin{proof}
    \leanok

    For $A$ in the double intersection and each $i$, the section $A(\omega^1)$ lies in
    $\bigcap_j m^2_j$, and $\omega^1 \mapsto \nu(A(\omega^1))$ is $m^1_i$-measurable; triviality
    of $\nu$ makes it $\set{0,1}$-valued, triviality of $\mu$ makes the set where it is $1$ of
    measure $0$ or $1$, and Fubini gives $(\mu \otimes \nu)(A) \in \set{0,1}$.
\end{proof}
...
\begin{example}[Georgii (7.15), stationary Markov chains]
    ...
    right-sided tail $\bigcap_{n} \mathcal{F}_{\mathbb{Z} \cap [n, \infty[} \subseteq \Tail$,
...
\begin{definition}[Finitary permutations and symmetric events, Georgii (7.16)]
    ...
    $\tau : \omega \mapsto (\omega_{\tau_* i})_{i \in \mathbb{N}}$; $|I_n| = n!$ and $I_n$
    increases with $n$.  The union $I = \bigcup_n I_n$ is the group of permutations of finitely
    many coordinates.  Put
    \[\mathcal{I}_n = \set{A \in \mathcal{F} : \tau^{-1}A = A \text{ for all } \tau \in I_n},
      \qquad \mathcal{I} = \bigcap_{n} \mathcal{I}_n,\]
    the σ-algebra of \textbf{symmetric events}; $(\mathcal{I}_n)$ is decreasing with infimum
    $\mathcal{I}$.
\end{definition}
...
\begin{lemma}[$(\gamma_n)$ is a proper consistent family of probability kernels]
    ...
    Each $\gamma_n$ is a proper probability kernel from $(\Cfg, \mathcal{I}_n)$ to
    $(\Cfg, \mathcal{F})$, and $\gamma_m \gamma_n = \gamma_n$ for $m \le n$.
\end{lemma}
\begin{proof}
    \leanok

    $\gamma_n(\Cfg \mid \omega) = |I_n|^{-1}|I_n| = 1$.  For $B \in \mathcal{I}_n$ the
    indicator $1_B$ is constant along the $I_n$-orbit of $\omega$, so
    $1_{A \inter B}(\tau\omega) = 1_B(\omega)1_A(\tau\omega)$ for all $\tau \in I_n$, which is
    properness.  Consistency for $m \le n$ is the count
...
\begin{definition}[The exchangeable abstract specification]
    ...
    Definition \ref{def:app-abstract-spec}, indexed by $\iota = \mathbb{N}$, with tail
    σ-algebra $\mathcal{I}$.
\end{definition}
...
\begin{definition}[Exchangeable distributions]
    ...
    A measure $\mu$ on $(\Cfg, \mathcal{F})$ is \textbf{exchangeable} (or symmetric) if
...
\begin{theorem}[Georgii (7.16)]
    ...
    n}$: the exchangeable probability measures are the invariant measures of the abstract
    specification of Definition \ref{def:app-exchangeable-spec}.
\end{theorem}
\begin{proof}
    \leanok

    $\mu\gamma_n(A) = |I_n|^{-1}\sum_{\tau \in I_n} \mu(\tau^{-1}A)$.  If $\mu$ is
    exchangeable each summand is $\mu(A)$.  Conversely, given $\mu\gamma_n = \mu$ and
    $\tau \in I_n$, apply the identity to $\tau^{-1}A$ and translate the sum over $I_n$ by
    $\tau$; the two sums coincide, so $\mu(\tau^{-1}A) = \mu(A)$.  Every $\tau \in I$ lies in
    some $I_n$.
\end{proof}
...
\begin{corollary}[Extreme exchangeable measures are trivial on $\mathcal{I}$]
    ...
    $\mathrm{ex}\,\Prob_I$ consists of those $\mu \in \Prob_I$ which are trivial on the
    σ-algebra $\mathcal{I}$ of symmetric events.
\end{corollary}
\begin{proof}
    \leanok

    Theorem \ref{thm:app-extreme-iff-tail-trivial} for the abstract specification of
    Definition \ref{def:app-exchangeable-spec}, whose tail σ-algebra is $\mathcal{I}$, together
...
\begin{proof}
    ...
    $[k,2k)$, independent of $B$ under $\mu$, so
...
\begin{definition}[Symmetrised product, empirical average, error term]
    ...
    For $a : \mathbb{N} \times \mathbb{N} \to [0,\infty]$ and $n \in \mathbb{N}$ put
...
\begin{theorem}[The symmetrisation estimate; Georgii's $|M_1(k,n)|/|M(k,n)| \to 1$]
    ...
    in $[0,\infty]$, and $\mathrm{symmErr}(k) \le k^2/(n-k) \to 0$ as $n \to \infty$ for fixed
    $k$.
\end{theorem}
\begin{proof}
    \leanok

    Induction on $k$ from the identity
    ...
    step, and $\sum_{j<k} a(k,\tau j) \le k$ gives the lower step; the accumulated errors sum to
    $\mathrm{symmErr}$.
\end{proof}
...
\begin{theorem}[Georgii (7.17), the substantial half, in coordinates]
    ...
    Let $\mu \in \mathrm{ex}\,\Prob_I$, with no hypothesis on $(E, \mathcal{E})$.  Then for
    all $A_0, \dots, A_{k-1} \in \mathcal{E}$
...
\begin{proof}
    ...
    Under $\gamma_n$ a box is the symmetrised product of the indicator rows
    $a(i,j) = 1_{A_i}(\omega_j)$, and $\gamma_n(\sigma_0 \in A_i \mid \omega)$ is the
    empirical average of the $i$-th row.  Since $\mu$ is trivial on $\mathcal{I}$
    (Corollary \ref{cor:app-extreme-exchangeable}), Lévy's downward theorem gives
    $\gamma_n(B \mid \cdot) \to \mu(B)$ $\mu$-a.s.\ for every measurable $B$.  Integrate the
    two-sided estimate of Theorem \ref{thm:app-symm-estimate} against $\mu$, using
    $\mu\gamma_n = \mu$ on the left and dominated convergence on the right, and let
...
\begin{proof}
    ...
    equal; by Theorem \ref{thm:app-boxes} $\mu$ agrees on boxes with the product of its
...
\begin{theorem}[Georgii (7.31), de Finetti's theorem in the version of Dynkin]
    ...
    Let $(E, \mathcal{E})$ be standard Borel.  Every exchangeable probability measure $\mu$ on
    $(E, \mathcal{E})^{\mathbb{N}}$ admits a representation
    \[\mu = \int_{\Prob(E, \mathcal{E})} \lambda^{\mathbb{N}}\, m(\mathrm{d}\lambda)\]
    with a unique probability measure $m$ on the measures on $E$ giving mass $0$ to the
...
\begin{proof}
    ...
    by Georgii (7.22), a unique probability weight $w$ on $\Prob_I$ carried by
    $\Prob_I \inter \Prob_{\mathcal{I}} = \mathrm{ex}\,\Prob_I$ with barycentre $\mu$.  By
    Theorem \ref{thm:app-7-17} every measure carried by $w$ is $\nu = (\sigma_1\nu)^{\mathbb{N}}$;
    pushing $w$ forward along $\nu \mapsto \sigma_1\nu$ gives $m$, and pushing $m$ forward
    along $\lambda \mapsto \lambda^{\mathbb{N}}$ recovers $w$.  The two maps are mutually
    inverse on the relevant carriers, so uniqueness of $w$ transfers to $m$.
\end{proof}
...
\begin{example}[Georgii (7.18)--(7.19), product specifications]
    ...
    Moreover $\Tail = \Tail^1 \times \Tail^2$ $\mu$-a.s.\ for every $\mu \in \Gibbs{\gamma}$.
\end{example}
...
\begin{remark}[Where (7.31) is used: Georgii's Example (2.27)]
    ...
    Let $E = \set{0,1}$, $S = \mathbb{N}$, $\lambda^y = y\delta_1 + (1-y)\delta_0$,
    $\mu^y = (\lambda^y)^{\mathbb{N}}$, and let $\gamma$ be the specification obtained by gluing
    the independent specifications $\ISSSD(\lambda^y)$ along the tail function
    $\xi = \liminf_n n^{-1}\sum_{i<n}\sigma_i$.  Every $\mu^x$, $x \in [0,1]$, is a Gibbs
    measure for $\gamma$, and since $\xi$ separates their supports, $\Gibbs{\gamma}$ is
    uncountable.  This $\gamma$ is not quasilocal; hence the quasilocality condition cannot be
    dropped from Georgii's Definition (8.6).

    Moreover $\Gibbs{\gamma} = \set{\int w(\mathrm{d}x)\,\mu^x : w \in \Prob([0,1])}$, which
    follows from $\mu\gamma_\Lambda = \mu$ alone; by de Finetti's theorem, Theorem
    \ref{thm:app-de-finetti}, this is the set of all exchangeable random fields on
    $\set{0,1}^{\mathbb{N}}$.
\end{remark}
