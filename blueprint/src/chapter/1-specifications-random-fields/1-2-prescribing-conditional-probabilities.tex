...
\begin{definition}[Specification]
    ...
    Following Georgii (1.23), a {\bf specification} is a family of kernels
    $\gamma : \Finset S \to \Ker_{\mathcal{F}_{S\setminus\Lambda}, \mathcal{E}^S}$ such that each
    $\gamma_\Lambda$ is a {\bf probability} kernel and is {\bf proper}, and which is {\bf consistent}:
    ...
    All three conditions are fields of the Lean structure: \texttt{isMarkovKernel'},
    \texttt{isProper'} and the consistency carried by \texttt{PreSpecification}. A family satisfying
    consistency alone is a \texttt{PreSpecification}.
\end{definition}
...
\begin{remark}[The probability and properness conditions]
    ...
    That $\gamma_\Lambda$ is a probability kernel, and that it is proper, are not properties some
    specifications lack: they are fields of the structure. \texttt{Specification.IsProper} names the
    condition, and \texttt{Specification.isProper} proves that \emph{every} specification satisfies
    it. Both are genuine hypotheses only on a \texttt{PreSpecification}.
\end{remark}
...
\begin{lemma}[Characterisation of Gibbs measures, Remark 1.24]
    ...
    Let $\gamma$ be a specification with parameter set $S$ and state space $(E, \mathcal{E})$, and let $\nu\in\mathfrak{P}(E^S, \mathcal{E}^S)$. TFAE:
    \begin{enumerate}
