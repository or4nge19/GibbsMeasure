...
\begin{definition}[Free measure of a specification]
    ...
    boundary condition $\eta$ and every $A \in \mathcal{F}_\Lambda$: inside a finite volume,
    $\gamma$ forgets the boundary condition and reproduces $\mu_0$. This is Georgii's
    $\lambda_\Lambda(A \mid \eta) = \lambda^S(A)$ of Remark (1.25).
\end{definition}
...
\begin{theorem}[Free measures determine the Gibbs measures]
    ...
    Let $\gamma$ have free measure $\mu_0 \in \Prob(\Cfg, \mathcal{F})$. Then every finite
    $\mu \in \Gibbs{\gamma}$ equals $\mu(\Cfg)\,\mu_0$; in particular $\Gibbs{\gamma}$ contains at
    most the one probability measure $\mu_0$. Moreover $\mu_0 \in \Gibbs{\gamma}$ if and only if
    $\mathcal{F}_\Lambda$ and $\mathcal{F}_{S \setminus \Lambda}$ are independent under $\mu_0$
    for every $\Lambda \in \Fin$. So $\Gibbs{\gamma}$ is $\set{\mu_0}$ under that independence
    and empty without it.
\end{theorem}
\begin{proof}
    \leanok
...
\begin{lemma}[Uniqueness of a Gibbs measure specified by an ISSSD]
    ...
    There is at most one Gibbs measure specified by $\ISSSD(\nu)$, and likewise for the
    inhomogeneous $\ISSSD$ of a family $(\nu_i)_{i \in S}$.
\end{lemma}
\begin{proof}
    \leanok
