...
\begin{lemma}[Null events of Gibbs measures, Remark 1.28.2]
    ...
    Let $\gamma = \rho\lambda$ be a $\lambda$-specification with everywhere positive densities
    $\rho_\Lambda$ over a $\sigma$-finite non-zero a priori measure.  A Gibbs measure
    $\mu \in \Gibbs{\gamma}$ annihilates an event $C \in \mathcal{F}_\Delta$, $\Delta \in \Fin$,
    exactly when the reference kernels $\lambda_\Delta(\,\cdot\mid\eta)$ all do.  In particular
    any two Gibbs measures for $\gamma$ have the same null events in
...
\begin{proposition}[Characterisation of modifiers, Proposition 1.30.1]
    ...
    \begin{enumerate}
        ...
        for every $\Lambda$ and $\eta$ (Georgii's standing normalisation
        $\lambda_\Lambda \rho_\Lambda = 1$ in (1.30)), and for all
    ...
    \end{enumerate}
    Without the normalisation conjunct the equivalence is false: $\rho_\Lambda \equiv 0$ satisfies
    the displayed identity but modifies no specification.
\end{proposition}
\begin{proof}
    % \uses{}
    % \leanok
...
\begin{proposition}[Characterisation of modifiers of independent specifications, Proposition 1.30.2]
    ...
    Let $\gamma$ be a specification satisfying
    $\gamma_{\Lambda_1} \circ_k \gamma_{\Lambda_2} = \gamma_{\Lambda_1 \cup \Lambda_2}$ for
    \emph{disjoint} volumes (Georgii's Notation (1.26); the Lean hypothesis is
    \texttt{IsDisjointlyConsistent}, satisfied by the independent specifications), and $\rho$ a
    family of functions. TFAE
    \begin{enumerate}
        \item $\rho$ is a modifier of $\gamma$
        ...
        $\gamma_{\Lambda_2 \setminus \Lambda_1}(\cdot|\alpha)$-almost all $\eta_2 : S \to E$, we have
