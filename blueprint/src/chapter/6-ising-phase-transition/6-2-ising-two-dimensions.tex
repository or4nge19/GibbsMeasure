...
\begin{lemma}[The Peierls estimate]
    ...
    Let $r(\beta) = \sum_{\ell \geq 1} \ell\, 4096^{\ell} e^{-2\beta\ell}$. For every $N$, every
    site $a$, and the cube $\Lambda = [-N,N]^2$ with $+$ boundary condition,
...
\begin{theorem}[Georgii Theorem (6.9), spontaneous magnetisation]
    ...
    $\beta \geq \tfrac12\log 9 = \log 3$ --- there are two distinct shift-invariant Gibbs measures
    $\mu_\pm \in \Gibbs{\gamma^{\beta}}$, exchanged by the spin flip, with
    \[\mu_+(\sigma_0 = -1) \;<\; \tfrac12 \;<\; \mu_-(\sigma_0 = -1).\]
    In particular $\Gibbs{\gamma^{\beta}}$ is not a singleton.
\end{theorem}
...
\begin{theorem}[Georgii Theorem (6.9), the low-temperature limit]
    ...
    Let $d$ be a metric for the topology of local convergence (Georgii Remark (4.3)(3)), let
    $\mathcal{G}_{\Theta}(\beta\Phi)$ be the set of shift-invariant Gibbs measures, and let
    $\delta_\pm$ be the Dirac measures at the two constant configurations. Then
...
\begin{lemma}[GKS and the finite-volume magnetisation]
    ...
    Let $\beta \geq 0$. The finite-volume $+$-boundary magnetisation
    $\langle\sigma_i\rangle_{\Lambda}^{+}(\beta) = 2\gamma^{\beta}_{\Lambda}(\sigma_i = 1 \mid +)-1$
    is a GKS correlation of the ferromagnetic interaction $\beta\Phi$; by Griffiths' inequalities
    it is nonnegative and nondecreasing in $\beta$, and it converges to $\mu_+^{\beta}(\sigma_i)$
    along $\Lambda \uparrow S$. The \textbf{spontaneous magnetisation} is
...
\begin{definition}[The critical inverse temperature]
    ...
    $\beta_c = \inf\set{\beta \geq 0 : |\Gibbs{\beta\Phi}| > 1}$. By Theorem \ref{thm:ising-lml}
    this is the threshold $\inf\set{\beta \geq 0 : m^*(\beta) > 0}$ of the order parameter, which
    is Georgii's definition.
\end{definition}
...
\begin{theorem}[Bounds on $\beta_c$]
    ...
    \[\operatorname{artanh}\tfrac14 \;\leq\; \beta_c \;\leq\; \log 3, \qquad
      \operatorname{artanh}\tfrac14 \approx 0.2554 .\]
    The lower bound is Dobrushin uniqueness (Theorem \ref{thm:dob-uniqueness}) applied to the
    $\mathbb{Z}^d$ Ising specification, for which the single-site criterion (8.9)(2)/(8.10) reads
    $2d\tanh|\beta J| < 1$; the upper bound is the sharpened Peierls threshold of Theorem
    \ref{thm:ising-69-two-phases}.
\end{theorem}
