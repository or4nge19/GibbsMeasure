...
\begin{theorem}[Georgii (17.11), one dimension]
    \label{thm:rp-17-11}
    \uses{lem:rp-17-9, def:rp-17-7}
    \lean{MeasureTheory.GibbsMeasure.wordProd, MeasureTheory.GibbsMeasure.sq_integral_wordProd_le, MeasureTheory.GibbsMeasure.integral_wordProd_shift, MeasureTheory.GibbsMeasure.abs_integral_prod_pow_le}
    \leanok

    Let $\mu$ be a finite measure on $E^{\mathbb Z_{2N}}$, invariant under the shift and under
    $\tilde r_1$, and $\tilde r_1$-positive. For bounded measurable $f_i$,
    $\bigl|\int \prod_i f_i(\omega_i)\,d\mu\bigr|^{2N} \leq
    \prod_j \int \prod_i f_j(\tau^i\omega_i)\,d\mu$, the right side being nonnegative.
\end{theorem}
\begin{proof}
    \leanok
    Lemma \ref{lem:rp-17-8} is hypothesis (ii) of Lemma \ref{lem:rp-17-9} for
    $D(\alpha) = |\int \prod_i \alpha_i(\omega_i)\,d\mu|$, and shift invariance is (i). The
    induction on the dimension and the coarse-graining (17.12)--(17.17) are not formalised.
\end{proof}
