...
This section is the only part of this file that is absent from `Challenge_Existence.lean`. It
identifies, for every absolutely summable potential `Φ`, the from-scratch objects defined above
with the corresponding objects of the `GibbsMeasure` library:

* `Bridge.hamiltonian_eq`: `H_Λ` is `Potential.hamiltonian`;
* `Bridge.freeMeasure_eq`: `λ_Λ^ω` is `Specification.isssd`;
* `Bridge.gibbsKernel_eq`: **the finite-volume Gibbs distribution written out above is literally
  the `Λ`-kernel of `Potential.gibbsSpecificationOfAbsolutelySummable`**;
* `Bridge.isGibbs_iff`: consequently the DLR equations above are the library's
